# Pro reference trainer (beta 0.2)

Simple local tool for practicing using your own images.

---

## Try online

<p>
  <a href="https://uri-strix.github.io/Pro-reference-trainer/">
    <img src="https://img.shields.io/badge/Open Demo-Click Here-00c2ff?style=for-the-badge" />
  </a>
</p>

## Preview

<p align="center" style="display: flex; justify-content: center; gap: 12px; flex-wrap: nowrap;">
  <img src="https://github.com/user-attachments/assets/6191a84b-ec63-469b-a5f9-1d043011e8bc" height="300"/>
  <img src="https://github.com/user-attachments/assets/8e133e3e-437e-433b-b1b4-73d668e493c5" height="300"/>
</p>


---

## Features

- Use **local images**
- Simple **timer-based training**
- Works in browser
- Adaptive UI (desktop / mobile)
- Language: russian. (english will be add soon)
- Single-file script

---


## Notes

- Works locally only  
- No external image sources  
- Beta version (0.2), may contain bugs  
